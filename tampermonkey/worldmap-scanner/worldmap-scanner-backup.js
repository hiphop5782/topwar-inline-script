// ==UserScript==
// @name         TopWar Unified Automation V2.12.15 - GitHub Request Reduction
...
(function () {
  ...
  const VERSION = "2.12.14-shared-token-flush-fix";
  ...
  async function ensureGithubToken(options = {}) {
    ...
    githubTokenEnsurePromise = (async () => {
      ...
      // 저장된 토큰은 실제 저장소 요청에서 검증되므로 /user를 자동 호출하지 않는다.
      // 토큰 입력 UI의 명시적 검증에서만 validateGithubToken을 사용한다.
